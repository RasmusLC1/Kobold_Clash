...
class Attack_Stategies():
    ...
    def Attack_Strategy(self) -> bool:
        if self.game.player.effects.invisibility.effect:
            return False
        if self.entity.distance_to_player < 300 and not self.looking_for_player:
            self.looking_for_player = 1
        
        if not self.Update_Player_Search():
            return False
    ...
    def Update_Player_Search(self):
        if not self.Line_Of_Sight(self.game.player.pos):
            self.looking_for_player += 1
            if self.looking_for_player > 200:
                self.looking_for_player = 0
                return True
        else:
            self.looking_for_player = 0 # set to 
        return False
    ...
    def Charge_player(self, distance):
        if self.looking_for_player < 200:
            # Check if the enemy has 
            if not self.Line_Of_Sight(self.game.player.pos):
                self.game.enemy_handler.Add_To_Pathfinding_Queue(self, self.game.player.pos)
                return False
