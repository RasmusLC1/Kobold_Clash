...
class Decoration(PhysicsEntity):
    ...
    def Damage_Taken(self, damage, effect):
        ...
        if effect == 'blunt':
            damage *= 2
        self.game.particle_handler.Activate_Particles(random.randint(2, 4), keys.loot_particle, self.rect().center, frame=random.uniform(0.7, 1.2))
    ...
    def Destroyed(self):
        ...
        self.game.particle_handler.Activate_Particles(random.randint(10, 15), keys.loot_particle, self.rect().center, frame=random.uniform(1, 2))
        return True
